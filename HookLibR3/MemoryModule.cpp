...
static BOOL
CopySections1(const unsigned char *data, size_t size, PIMAGE_NT_HEADERS old_headers, PMEMORYMODULE module)
{
	int i, section_size;
	unsigned char *codeBase = module->codeBase;
	unsigned char *dest;
...
ExecuteTLS(PMEMORYMODULE module)
{
    ...
    if (callback) {
        while (*callback) {
            (*callback)((PVOID) codeBase, DLL_PROCESS_ATTACH, NULL);
            callback++;
        }
    }
    return TRUE;
...
HMEMORYMODULE MemoryLoadLibrary1(const void *data, size_t size,CHAR ** buffer,int * len)
{
	return MemoryLoadLibraryEx1(data, size, MemoryDefaultAlloc, MemoryDefaultFree, MemoryDefaultLoadLibrary, MemoryDefaultGetProcAddress, MemoryDefaultFreeLibrary, NULL, buffer, len);
}

HMEMORYMODULE MemoryLoadLibraryEx1(const void *data, size_t size,
	...
	CHAR ** buffer, int * len)
{
	...
	code = (unsigned char *)malloc(alignedImageSize);
	...
	memset(code, 0,alignedImageSize);
	...
	if (!CopySections1((const unsigned char *)data, size, old_header, result)) {
		goto error;
	}
...
//	 TLS callbacks are executed BEFORE the main loading
	...
	*buffer = (char*)result->codeBase;
	*len = result->headers->OptionalHeader.SizeOfImage;
